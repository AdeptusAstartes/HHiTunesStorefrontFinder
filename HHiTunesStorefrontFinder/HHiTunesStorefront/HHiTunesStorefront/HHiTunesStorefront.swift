...
public class HHiTunesStorefront: NSObject {
    ...
    let countryData: Array<NSDictionary> = [
    ...
    override init() {
        ...
        countryCode = locale.objectForKey(NSLocaleCountryCode) as? NSString;
        countryName = locale.displayNameForKey(NSLocaleCountryCode as NSString, value: countryCode!);
        
        super.init();
        
        let filteredCountry: NSDictionary = countryData.filter(matchesCountryCode).first!;
        storefrontId = filteredCountry["storefrontId"] as? NSString;

    }
    
    private func matchesCountryCode(country: NSDictionary) -> Bool {
